...
externbidConfig* configureObject::parse_exBid_config(string &str, int config_item_num)
{
    try
    {
        size_t str_size = str.size();
        ...
        if(parseBraces(dst, subStrList)==true)
        {
            ...
            string& str_id = subStrList.at(0);
            string& str_ip = subStrList.at(1);
            string& str_port = subStrList.at(2);
            string& str_connectCnt = subStrList.at(3);
            string& str_protocol = subStrList.at(4);
            string& str_format = subStrList.at(5);
            if((is_number(str_id) == false)||(is_number(str_port) == false)||(is_number(str_connectCnt) == false))
            {
                g_manager_logger->error("throttle config format error: is not number");
                return NULL;
            }
            int     number_id = atoi(str_id.c_str());
            int     number_port = atoi(str_port.c_str());
            int     number_connectCnt = atoi(str_connectCnt.c_str());

            const unsigned short connect_max = 0xFFFF;
            if((number_port>short_max)||(number_connectCnt>connect_max))
            {
                g_manager_logger->error("port or connectCnt out of range");
                return NULL;
            }
         
            externbidConfig *bidder = new externbidConfig(number_id, str_ip, (unsigned short) number_port,
                              (unsigned short)number_connectCnt,  str_protocol, str_format);
            return bidder;
        }
        return NULL;
    }
...
void configureObject::parse_throttle_info(vector<string>& textList)
{
    string value;
    vector<string> config_block;
    get_configBlock("throttleConfig", textList, config_block);
...
void configureObject::parse_bidder_info(vector<string> &textList)
{
try
{
    vector<string> config_block;
    get_configBlock("bidderConfig", textList, config_block);
...
void configureObject::parse_exBid_info(vector<string> &textList)
{
try
{
    vector<string> config_block;
    get_configBlock("externBidConfig", textList, config_block);
    const int item_num = 6;
    for(auto it = config_block.begin(); it != config_block.end(); it++)
    {
        string &str = *it;
        externbidConfig *bidder = parse_exBid_config(str, item_num);
        if(bidder)
        {
            m_exbid_info.add_bidderConfig(bidder);
        }
    }
}
catch(...)
{
    g_manager_logger->emerg("parse_exBid_info exception");
    exit(1);
}
}
...
void configureObject::parse_connector_info(vector<string> &textList)
{
    try
    {
        vector<string> config_block;
        get_configBlock("connectorConfig", textList, config_block);
...
void configureObject::parse_bc_info(vector<string> &textList)
{
try
{
    vector<string> config_block;
    get_configBlock("bcConfig", textList, config_block);
    const int item_num = 5;
    for(auto it = config_block.begin(); it != config_block.end(); it++)
    {
        string &str = *it;
...
void configureObject::readConfig()
{
    try
    {
        ...
        while(getline(m_infile, buf))
        {
            ...
            else if(string_find(buf, "externBidInfo"))
            {
                vector<string> value;
                if(get_block(m_infile, buf, value) == false) throw 0;
                parse_exBid_info(value);
            }
...
void configureObject::display() 
{
    ...
    m_bidder_info.display();
    m_connector_info.display();
    m_exbid_info.display();
    m_bc_info.display();
