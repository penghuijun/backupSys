...
void configureObject::readConfig()
{
    try
    {
        ...
        while(getline(m_infile, buf))
        {
            ...
            if(buf.compare(0, 17, "bidderCollectorIP")==0)
            {
            ...
            else if(buf.compare(0, 8, "exBidder")==0 && ((symIdx = buf.find('{')) != string::npos) )
            {
                block_started = true;
            }
        }//while
...
void configureObject::display() const
{
    ...
    cout << "config throttle worker num: " << get_bidderworkerNum()<< endl;
