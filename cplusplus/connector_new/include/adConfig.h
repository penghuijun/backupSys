...
public:
	...
	void display(shared_ptr<spdlog::logger>& configLog)
	{
			configLog->info("{0:d}\t{1}\t{2:d}\t{3:d}\t{4:d},{5:d}",get_throttleID(), get_throttleIP(), get_throttleAdPort()
...
public:
	...
	void display(	shared_ptr<spdlog::logger>& configLog)
	{
		configLog->info("======throttle infomation begin======");
		configLog->info("id\tip\t\tadport\tpubport\tmanaPort\tworkerNum");

		for(auto it = m_throttleConfigList.begin(); it != m_throttleConfigList.end(); it++)
		{
			throttleConfig *throttle = *it;
			if(throttle) throttle->display(configLog);
		}
		configLog->info("======throttle infomation end======");
	}
...
public:
	...
	void display(shared_ptr<spdlog::logger>& configLog)
	{
		configLog->info("{0:d}\t{1}\t{2:d}\t{3:d}\t{4:d},{5:d}",get_bidderID(), get_bidderIP(), get_bidderLoginPort()
...
public:
	...
	void display(shared_ptr<spdlog::logger>& configLog)
	{
	    configLog->info("======bidder infomation begin======");
	    configLog->info("id\tip\t\tloginport\tworkerNum\tthreadPoolsize\tcoverceNum");
		for(auto it = m_bidderConfigList.begin(); it != m_bidderConfigList.end(); it++)
		{
			bidderConfig *bidder = *it;
			if(bidder) bidder->display(configLog);
		}
		configLog->info("bidder target redis: {0},{1:d}", m_redis_ip, m_redis_port);
	    configLog->info("======bidder infomation end======");
	}
	...
	vector<bidderConfig*>& get_bidderConfigList() {return m_bidderConfigList;}
	~bidderInformation()
	{
...
{
public:
	mysqlConfig(int id,const string& ip,unsigned short dataport,const string& name,const string& pwd)
	{
		mysqlID = id;
		mysqlIP = ip;
		mysqlDataPort = dataport;
		mysqlUserName = name;
		mysqlUserPwd = pwd;		
	}
	const string& get_mysqlIP() const {return mysqlIP;}
	int get_mysqlDataPort() const {return mysqlDataPort;}
	const string& get_mysqlUserName() const {return mysqlUserName;}
	const string& get_mysqlUserPwd() const {return mysqlUserPwd;}
	~mysqlConfig(){}
private:
	int            mysqlID;
	string         mysqlIP;
	unsigned short mysqlDataPort;
...
public:
	...
	void display(const shared_ptr<spdlog::logger>& configLog) const
	{
		configLog->info("{0:d}\t{1}\t{2:d}\t{3:d}\t{4:d}",get_connectorID(), get_connectorIP(), get_connectorManagerPort()
...
public:
	...
	void add_mysqlConfig(mysqlConfig* mysql)
	{
		m_mysqlConfigList.push_back(mysql);
	}	
	...
	void display(const shared_ptr<spdlog::logger>& configLog) const
	{
	    configLog->info("======connector infomation begin======");
	    configLog->info("id\tip\t\tloginport\tworkerNum\tthreadPoolsize\tcoverceNum");
		for(auto it = m_connectorConfigList.begin(); it != m_connectorConfigList.end(); it++)
		{
			connectorConfig *connector = *it;
			if(connector) connector->display(configLog);
		}
		configLog->info("connector redis: {0},{1:d}", m_redis_ip, m_redis_port);
	    configLog->info("======connector infomation end======");
	}
	...
	void   set_logLevel(int level){m_logLevel = level;}
	vector<connectorConfig*>& get_connectorConfigList() {return m_connectorConfigList;}
	vector<mysqlConfig*>& get_mysqlConfigList() {return m_mysqlConfigList;}	
	~connectorInformation()
	{
...
private:
	...
	vector<mysqlConfig*>	 m_mysqlConfigList;	
	string                   m_redis_ip;
	unsigned short           m_redis_port;
	int                      m_logLevel = 0;
};
...
public:
	...
	void display(shared_ptr<spdlog::logger>& configLog)
	{
		configLog->info("{0:d}\t{1}\t{2:d}\t{3:d}\t{4:d}",get_bcID(), get_bcIP(), get_bcDataPort()
...
public:
	...
	void display(shared_ptr<spdlog::logger>& configLog)
	{
	    configLog->info("======bc infomation begin======");
	    configLog->info("id\tip\t\tdataPort\tmanagerPort\tthreadPoolSize");
		for(auto it = m_bcConfigList.begin(); it != m_bcConfigList.end(); it++)
		{
			bcConfig *bc = *it;
			if(bc) bc->display(configLog);
		}
	    configLog->info("bc redis :{0}, {1:d}, {2:d}", m_redis_ip, m_redis_port, m_redis_timeout);

	    configLog->info("======bc infomation end======");
	...
	vector<bcConfig*>& get_bcConfigList() {return m_bcConfigList;}
...
public:
	...
	mysqlConfig* parse_mysql_config(string &str, int config_item_num);

	bcConfig*       parse_bc_config(string &str, int item_num);
	void parse_throttle_info(vector<string>& orgin_text);
	void parse_bidder_info(vector<string>& orgin_text);
	void parse_connector_info(vector<string> &orgin_text);
	void parse_bc_info(vector<string>& orgin_text);

 	throttleInformation& get_throttle_info() {return m_throttle_info;}
    bidderInformation&   get_bidder_info() {return m_bidder_info;}
	bcInformation&       get_bc_info() {return m_bc_info;}
	connectorInformation& get_connector_info(){ return m_connector_info;}
	...
	int 				 get_connectorLevel(){return m_connector_info.get_logLevel();}
	string&              get_vast_businessCode(){return m_vastBusiCode;}
	string&              get_mobile_businessCode(){return m_mobileBusiCode;}
	string&              get_logRedisIP() {return m_logRedisIP;}
	unsigned short       get_logRedisPort(){return m_logRedisPort;}
	...
	void clear()
	{
		...
		m_connector_info.connectorInformation_erase();
	}
	~configureObject()
	{
		m_infile.close();
