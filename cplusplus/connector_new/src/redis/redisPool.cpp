...
int* redisClient::target_set_intersection(int *setA, int setA_number, int *setB, int setB_number, int &result_cnt)
{
    ...
    if(sum==0) return NULL;
    int *oper_result = new int[sum];
    //泛型算法set_intersection，求出集合A和集合B相同的元素放到oper_result里
...
{
    result_cnt = 0;
    if((byte_array == NULL) ||(byte_cnt == 0)) 
    {
        cout << "(byte_array == NULL) ||(byte_cnt == 0)" << endl;
        return NULL;
    }
        
    int campaignID_number = byte_cnt/4;
    int *int_array = new int[campaignID_number];
    cout<<"<<<<<< campaignID_number: " << campaignID_number <<endl;    
...
bool redisClient::target_set_convergence(list<target_result_info*>& target_result_list,operationTarget &target_obj, int convergence_num, target_result_info &target_result)
{
    ...
    cout << "<<<<<< result_item_size = " << result_item_size << endl;
...
{   
    ...
    //redisAppendCommand函数只是将待发送的命令写入到上下文对象的输出缓冲区中，直到调用后面的
    //redisGetReply命令才会批量将缓冲区中的命令写出到Redis服务器。这样可以有效的减少客户端与
    //服务器之间的同步等候时间，以及网络IO引起的延迟。

    //Redis HGET命令用于获取与字段中存储的键哈希相关联的值
    //HSET myhash field1 "foo"
    //HGET myhash field1  ==> 输出: "foo"
    
    for(auto it = target_geo.begin(); it != target_geo.end(); it++) // 3次{name=country,id=47} {name=region,id=433} {name=city,id=14150}
    {
       target_infomation *info = *it;
       if(!info) continue;
       cout << "<<<< hget " << info->get_name().c_str() << " " << info->get_id().c_str() << endl;
       /*<<<< hget country 47
                <<<< hget region 433
                <<<< hget city 14150*/
       if(redisAppendCommand(m_context, "hget %s %s", info->get_name().c_str(), info->get_id().c_str()) != REDIS_OK)
       {
           redis_free();        //redisFree(m_context);释放上下文
           redis_connect();     //重连
           return false;
       }
       geo_num++;
    ...
    for(auto it = target_os.begin(); it != target_os.end(); it++)
    {
     ...
     for(auto it = target_signal_vec.begin(); it != target_signal_vec.end(); it++)
     {
      ...
      {
         vector<int> tmp_vec;
         //执行m_context缓存里面的命令，返回对象ply,
         //由于后面重复使用该变量，所以需要释放，否则内存泄漏。
         redisReply *ply;           
    ...
    //输出target_item_list
    auto it  = target_item_list.begin();
    cout<< "------------ target_item_list start---------------" << endl;
    for(it = target_item_list.begin(); it != target_item_list.end(); it++)
    {
        target_result_info *item = *it;
        if(item)
        {
            cout<<"m_name = "<<item->get_name()<<endl;
            int *ptr = item->get_array_ptr();
            for(int i=0;i<item->get_array_size();i++)
            {
    ...
    cout<< "------------ target_item_list end-----------------" << endl;
    int convergence_num = conver_num; //convergence_num's value = 0;
    cout<<"<<<<<<<< convergence_num = "<< convergence_num << endl;
    //将redis里面获取的值，保存在tar_result_info里面
